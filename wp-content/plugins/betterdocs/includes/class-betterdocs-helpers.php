...
class BetterDocs_Helper
{
    ...
    public static function list_query_arg($post_type, $multiple_kb, $tax_slug, $posts_per_grid, $order_post, $order = 'ASC')
    {
        $args = array(
            'post_type'   => $post_type
        );
        ...
        if ($order_post != 'betterdocs_order' || $order_post != 'none') {
            if ($order_post == '1') {
                $args['orderby'] = 'title';
            } else {
                $args['orderby'] = $order_post;
            }
            $args['order'] = $order;
        }
        return $args;
    ...
    public static function child_taxonomy_terms($term_id, $multiple_kb, $orderby = "name", $order = "")
    {
        global $wp_query;
        $terms_object = array(
            'parent' => $term_id
